...
# 創建labels資料夾
if not os.path.exists('labels_crop_obb'):
    os.makedirs(os.path.join(root_dir, folder,'labels_crop_obb'))

# 解析COCO JSON文件
images = {image['id']: image for image in coco_data['images']}
categories = {category['id']: category for category in coco_data['categories']}

def get_rotated_box(bbox, angle, image_width, image_height):
    new_image_width = 800
...
for annotation in coco_data['annotations']:
    ...
    # 計算YOLO格式所需的旋轉bounding box頂點座標
    rotated_bbox = get_rotated_box(bbox, angle, image_width, image_height)
    
    # 取得對應的class
    class_id = category_id  # 如果需要使用class名稱，可以改為categories[category_id]['name']
    
    # Create YOLO格式的標註
    yolo_annotation = f"{class_id} " + " ".join([f"{x} {y}" for x, y in rotated_bbox])
    
    # 取得對應的圖片文件名，並create label's 文件名
    image_filename = image_info['file_name']
    annotation_filename = os.path.splitext(image_filename)[0] + '.txt'
    
    # 將標註寫入對應的txt文件中
    with open(os.path.join(root_dir, folder,'labels_crop_obb', annotation_filename), 'a') as f:
